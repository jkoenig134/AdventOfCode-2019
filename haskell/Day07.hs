...
run codes ip input output
  | op == 3   = run (set codes (codes !! (ip + 1)) (head input)) (ip + 2) (tail input) output
...
amplifierRun :: [Int] -> [Int] -> Int
amplifierRun codes (a:b:c:d:e:xs) = runE
  where
    runE = head $ mainRun codes [e, runD]
    runD = head $ mainRun codes [d, runC]
    runC = head $ mainRun codes [c, runB]
    runB = head $ mainRun codes [b, runA]
    runA = head $ mainRun codes [a,0]

maxAmplifier :: [Int] -> Int
maxAmplifier codes = fst $ maximum [(amplifierRun codes [a,b,c,d,e], [a,b,c,d,e]) | a <- [0..4], b <- [0..4], c <- [0..4], d <- [0..4], e <- [0..4], duplicated [a,b,c,d,e]]
   where
     duplicated list = sum (map (count list) list) == (length list)
     
{- Handle input and solve executions -}

-- Solve first challenge
solve1 :: Input -> Int
solve1 (Line (Just line)) = maxAmplifier (toIntList line)
...
main = do
  solve "Max thruster signal"           (Line Nothing) (solve1)
